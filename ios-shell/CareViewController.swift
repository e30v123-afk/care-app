...
/// يحقن قشرة التطبيق (shell.js) في كل صفحة تُحمَّل من careksa.com،
...
///    ما لم يحتوِ وسم viewport على viewport-fit=cover، وموقع careksa.com لا يحتويه.
///    لذا نُبلّغ الويب بقيم المنطقة الآمنة الحقيقية كمتغيّرات CSS.
class CareViewController: CAPBridgeViewController {
    ...
    private func injectShell() {
        guard let controller = webView?.configuration.userContentController else {
            CAPLog.print("Care: لا يوجد userContentController")
            return
        }
        guard let url = Bundle.main.url(forResource: "shell", withExtension: "js"),
              let source = try? String(contentsOf: url, encoding: .utf8) else {
            CAPLog.print("Care: تعذّر العثور على shell.js في حزمة التطبيق")
            return
        }
